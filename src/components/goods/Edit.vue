 ...
 * @Version: 2.0
 * @Autor: Seven
 * @Date: 2021-04-05 19:56:36
 * @LastEditors: Seven
 * @LastEditTime: 2021-04-06 16:33:12
-->
<!--
 * @Description: 
 * @Version: 2.0
 ...
 * @LastEditors: Seven
 * @LastEditTime: 2021-04-05 19:52:54
-->
<template>
  <div>
...
export default {
  ...
  methods:{
    ...
    handleRemove(file,fileList){
      ...
      const idx= this.editForm.pics.findIndex(item=>{
        item.pic===filePath
      })
      this.editForm.pics.splice(idx,1)
    },
    /**
     * @description: 图片上传成功
    ...
    editGoods(){
      this.$refs.editFormRef.validate(async valid=>{
        ...
        // const form=editForm 浅拷贝
        const form=_.cloneDeep(this.editForm)//深拷贝 lodash
    ...
    /**
     * @description: 根据id发送请求  存储数据数据回写 对商品分类进行处理
     *               对回写图片进行处理
     * @param {this.$touter.query.id} list页面编辑传过来的id
     * @return {*}
     * @Date: 2021-04-06 14:39:33
     * @author: Seven
     */
    async getGoodsById(){
      ...
      //将cat转成数组 将数组里每一项转成number
      res.data.goods_cat=res.data.goods_cat.split(',').map(item=>{
        return item*1
      })
      //res.data.pics里的每一项都存储进fileList数据仓库里 同时对url进行修改
      //增加一个pic属性
